...
For custom form-level error summaries (banners, toasts), listen to `form:invalid` or use `onFormInvalid` in `FormControllerOptions`. See [Error Rendering](/guides/error-rendering/#form-level-error-summary).
...
## Example: Submit Button Loading State
...
See [Loading State](/guides/loading-state/) for full details.
